/**
 * BC 115 — CandidateClusterRadarVsRole
 * BC 170 — ARIA labels on SVG + aria-live announcer
 *
 * Radar chart: candidate cluster scores vs role target scores.
 * Used on the workforce-app candidate review panel.
 *
 * Props:
 *   data  — two series, each with 8 values (C1..C8, 0..100) and a label
 *             [0] = candidate scores, [1] = role targets
...
const SERIES_COLORS = [
  { stroke: '#8B5CF6', fill: 'rgba(139, 92, 246, 0.18)', text: '#6D28D9' },  // violet — candidate
  { stroke: '#F59E0B', fill: 'rgba(245, 158, 11, 0.18)',  text: '#B45309' },  // amber  — role targets
]

export function CandidateClusterRadarVsRole({
  data,
  size = 360,
}: {
  ...
  useEffect(() => {
    ...
    const candidateAvg = Math.round(data[0].data.reduce((s, v) => s + v, 0) / data[0].data.length)
    const roleAvg = Math.round(data[1].data.reduce((s, v) => s + v, 0) / data[1].data.length)
    liveRef.current.textContent =
      `Radar chart updated. ${data[0].label} average: ${candidateAvg}. ${data[1].label} average: ${roleAvg}.`
  }, [data])
