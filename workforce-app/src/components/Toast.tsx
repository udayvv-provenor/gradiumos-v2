...
export function Toaster() {
  ...
  useEffect(() => {
    ...
    listeners.push(h)
    return () => { listeners = listeners.filter(f => f !== h) }
  }, [])
  return (
    <div className="fixed top-4 right-4 z-50 flex flex-col gap-2 pointer-events-none">
      {toasts.map(t => (
        <div key={t.id} className={`pointer-events-auto px-4 py-3 rounded-md text-sm font-medium shadow-modal ${t.type === 'success' ? 'bg-green-700 text-white' : 'bg-navy text-white'}`}>
          {t.message}
        </div>
      ))}
    </div>
