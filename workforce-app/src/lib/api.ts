...
export async function postSignupInstitution(body: {
  institutionName: string; type: string; email: string; password: string; name: string;
...
// v3.1 — `archetype` dropped. Server derives from first JD upload (mode of all
// roles' JD-extracted archetypes, recomputed on every JD upload).
