    ...
    <div className="min-h-screen bg-cloud flex items-center justify-center p-4">
      <div className="w-full max-w-sm">
        <div className="mb-8 text-center">
          ...
          <p className="text-sm text-slate mt-1">Sign in to your employer account</p>
        </div>
        <div className="bg-white rounded-lg border border-rule shadow-card p-6">
          ...
          <form onSubmit={handleSubmit} className="flex flex-col gap-4">
            <div>
              ...
              <input type="email" required value={form.email} onChange={e => setForm(f => ({ ...f, email: e.target.value }))} className="w-full text-sm px-3 py-2 border border-rule rounded focus:outline-none focus:border-accent transition-colors" placeholder="ta@company.com" />
            </div>
            <div>
          ...
          <p className="text-center text-xs text-slate mt-4">New employer? <Link to="/signup" className="text-accent font-medium hover:underline">Create account</Link></p>
        </div>
      </div>
    </div>
  )
