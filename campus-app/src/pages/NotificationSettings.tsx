/**
 * BC 133 — Notification Settings (/settings/notifications) — Campus portal
 *
 * Per-event toggle for campus Dean/Placement Officer notification preferences.
...
const EVENTS: EventDef[] = [
  { key: 'partnership_request',  label: 'Partnership requests',   description: 'When an employer requests a partnership with your institution.',         transactional: false },
  { key: 'sponsored_pathway',    label: 'Sponsored pathways',     description: 'When an employer funds a pathway for your institution.',                  transactional: false },
  { key: 'suspicious_attempt',   label: 'Flagged assessments',    description: 'When a learner attempt is flagged for AI-authored or proctoring issues.', transactional: false },
]

const STORAGE_KEY = 'notification_prefs_campus'
    ...
    <div className="max-w-xl mx-auto">
      ...
      <div className="bg-white rounded-xl border border-rule shadow-card overflow-hidden mb-4">
        <div className="px-5 py-3 border-b border-rule bg-cloud/60">
          <div className="text-xs font-semibold text-slate uppercase tracking-wide">Campus notifications</div>
        </div>
        {EVENTS.map((evt, i) => (
