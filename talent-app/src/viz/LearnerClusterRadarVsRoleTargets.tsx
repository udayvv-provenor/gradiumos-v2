/**
 * BC 115 — LearnerClusterRadarVsRoleTargets
 * BC 170 — ARIA labels on SVG + aria-live announcer
 *
 * Radar chart: learner cluster scores vs role target bar.
 * Used on the talent-app career-fit and opportunity detail screens.
 *
 * Props:
 *   data  — two series, each with 8 values (C1..C8, 0..100) and a label
 *             [0] = learner scores, [1] = role target bar
...
const SERIES_COLORS = [
  ...
  { stroke: '#3B82F6', fill: 'rgba(59, 130, 246, 0.18)',  text: '#1D4ED8' },  // blue   — role bar
]

export function LearnerClusterRadarVsRoleTargets({
  data,
  size = 360,
}: {
  ...
  // BC 170 — aria-live announcer text
  const liveRef = useRef<HTMLSpanElement>(null)
  useEffect(() => {
    if (!liveRef.current) return
    const learnerAvg = Math.round(data[0].data.reduce((s, v) => s + v, 0) / data[0].data.length)
    const roleAvg = Math.round(data[1].data.reduce((s, v) => s + v, 0) / data[1].data.length)
    liveRef.current.textContent =
      `Radar chart updated. ${data[0].label} average: ${learnerAvg}. ${data[1].label} average: ${roleAvg}.`
  }, [data])
  ...
  // Build a concise title describing current data
  const titleText = `Radar chart: ${data[0].label} cluster scores vs ${data[1].label} targets`

  return (
    <div className="flex flex-col items-center">
      {/* BC 170 — visually hidden live region announces data changes */}
      <span
        ref={liveRef}
        aria-live="polite"
        aria-atomic="true"
