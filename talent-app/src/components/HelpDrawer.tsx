/**
 * BC 171 + BC 172 — HelpDrawer (Talent portal)
 *
 * Slide-in panel from the right, triggered by the ? button in the header.
 * Three tabs: Docs (FAQ), Contact (feedback form), Status (health check).
 *
 * BC 172 feedback endpoint: POST /api/v1/feedback
 * BC 170 aria: aria-label="Help", aria-expanded on trigger (set in Layout)
 */
  ...
  const faqs: { q: string; a: string }[] = [
    {
      ...
      a: 'Go to Assessments in the left sidebar. Select a cluster, pick a question bank, and press "Start". Your answers are scored automatically and update your cluster confidence in real time.',
    },
    {
      q: 'What is the GradiumOS Signal?',
      a: 'Your Signal is a verified competency score computed from all your assessment results across 8 clusters (C1–C8). Once enough confidence is built it publishes as a portable, signed credential employers can verify at /verify.',
    },
    {
      q: 'How do I apply for a role?',
      a: 'Open Opportunities, browse roles sorted by match score, and click "Apply". Your Signal is attached automatically. You can track status on the Applications page.',
    },
    {
      q: 'What does "Near miss" mean on an opportunity?',
      a: 'Near miss means you are within a small delta of meeting the role\'s cluster bar. Click "Close gap" to open the targeted lesson for that cluster and close the gap faster.',
    },
    {
      q: 'How do I upload my resume?',
      a: 'Go to Profile and use the Resume upload card. Uploading unlocks personalised gap analysis and AI-augmented learning paths in the Learn section.',
    },
  ]
    ...
    <div className="space-y-3">
      ...
      >
        <span
          ...
          className={clsx(
            ...
            isOk ? 'bg-green-500' : 'bg-amber-400',
          )}
        />
      ...
      {(health.version || health.commit) && (
        <div className="text-xs text-slate space-y-1 px-1">
          {health.version && (
            <div>
              <span className="font-semibold text-navy">Version:</span> {health.version}
            </div>
          )}
          {health.commit && (
            <div>
              <span className="font-semibold text-navy">Commit:</span>{' '}
              <span className="font-mono">{health.commit.slice(0, 8)}</span>
            </div>
          )}
        </div>
      )}
...
export interface HelpDrawerProps {
  open: boolean
  onClose: () => void
  /** Which portal is hosting this drawer — controls FAQ content */
    ...
    <>
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto px-5 py-5">
          ...
          <div
            id="help-panel-status"
            role="tabpanel"
            aria-labelledby="help-tab-status"
            hidden={tab !== 'status'}
          >
